...
public class Wand {
   ...
   public String getName(){//getメソッドname
      return this.name;
   }
   public void setName(String name){//setメソッドname
      if(name == null || name.length() < 3){//短かすぎ nullもいれる nullが先じゃないとエラーになる
         throw new IllegalArgumentException("名前が短かすぎる。処理を中断");
     }
     this.name = name;//検査完了。代入しても大丈夫
   }

   public double getPower(){//getメソッドpower
      return this.power;
   }
   public void setPower(double power){//setメソッドpower //②解答
